...
integration はアプリ内 port、共通 schema、dependency、provider 実装を分ける。provider 実装は `*_provider` ディレクトリ（SlotKeeper は `jwt_provider`）に閉じる。
...
- API operation から `src/app/integrations/{resource}/*_provider/client.py` を import しない。
...
| INTEGRATION-DONT-002 | MUST NOT | `functions_no_provider_imports` | - | API operation から `src/app/integrations/{resource}/*_provider/client.py` を import しない。 |
