...
予約者本人または管理者が、開始前の確定予約を版つきで取消します。取消と取消履歴は同じtransactionで確定します。
...
| 項目 | 型 | 必須 | 説明 | 制約 |
| --- | --- | --- | --- | --- |
| `version` | `integer` | yes | 取消前に取得した予約の版です。 | minimum=1.0 |
...
| `403` | 呼び出し元が予約者本人でも管理者でもない場合。 | `application/json` | 13 field(s) |
| `404` | 指定された予約が存在しない場合。 | `application/json` | 13 field(s) |
| `409` | 予約の版が古い、取消済み、または開始済みの場合。 | `application/json` | 13 field(s) |
...
| `error.details[].resource.reservationId` | `string \| null` | no | 取消対象予約の存在確認、再取得、問い合わせに使用する予約IDです。 | minLength=1, maxLength=36 |
| `error.details[].resource.version` | `integer \| null` | no | 取消要求で指定した予約の版です。 | minimum=1 |
