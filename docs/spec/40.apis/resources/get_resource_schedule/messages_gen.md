...
# Message catalog: `resources/get_resource_schedule`
...
| :--- | :--- |
| domain | `resources` |
| api | `get_resource_schedule` |
| routes | GET /resources/{resourceId}/schedule (getResourceSchedule) |
| router | `src/app/apis/resources/get_resource_schedule/router.py` |
| messages | 1 |
...
| id | message_id | ログ概要 |
| :--- | :--- | :--- |
| `M001` | `getResourceSchedule.router_api_function_error` | Routerで捕捉したApiFunctionErrorにより予約表取得が失敗した。 |

## ログ詳細

### `M001` `getResourceSchedule.router_api_function_error`
...
| status |  |
| wrapper calls | 1 |
| ログ概要 | Routerで捕捉したApiFunctionErrorにより予約表取得が失敗した。 |
| 説明 | ROUTER_HANDLED_EXCEPTIONSを捕捉した場合。 |
| 対応すべきこと | 同一routeの5xx率、直近deploy、DB状態を確認する。 |
| runbook | RUNBOOK-unexpected-api-failure |
| 実装参照 | src/app/apis/resources/get_resource_schedule/functions.py<br>wrapper: src/app/apis/resources/get_resource_schedule/functions.py (ops_logger.error) |
...
| `resource.resourceId` | `string \| null` | 予約表の対象資源の存在確認、問い合わせに使用する資源IDです。 |
| `resource.day` | `CalendarDate \| null` | 予約表を再取得するための日付です。 |
