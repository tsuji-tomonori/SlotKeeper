...
資源と日本時間の日付を指定して予約済み時間帯を取得します。他人の予約は時間帯と「予約済み」だけを返します。
...
| `day` | `string` | yes | 日本時間で予約表を表示する日付です。 | - |
...
| `422` | pathまたはqueryが型や制約に一致しない、または継続tokenが不正な場合。 | `application/json` | 13 field(s) |
...
| 項目 | 型 | 必須 | 説明 | 制約 |
| --- | --- | --- | --- | --- |
| `items` | `array<ScheduleSlotResponse>` | yes | 一覧レスポンスに含まれるリソース配列です。 | - |
| `items[].startAt` | `string` | yes | 予約済み時間帯の開始日時です。 | - |
| `items[].endAt` | `string` | yes | 予約済み時間帯の終了日時です。この時刻を含みません。 | - |
| `items[].label` | `string` | yes | 共有予約表に表示する固定の表示名です。 | - |
| `items[].reservation` | `ScheduleReservationResponse \| null` | no | 呼び出し元が予約者本人または管理者の場合だけ返す予約の詳細です。 | - |
| `items[].reservation.reservationId` | `string` | yes | 予約を一意に識別するIDです。 | minLength=1, maxLength=36 |
| `items[].reservation.resourceId` | `string` | yes | 予約対象の資源を一意に識別するIDです。 | minLength=1, maxLength=36 |
| `items[].reservation.ownerPrincipalId` | `string` | yes | 予約者を表す認証主体IDです。 | minLength=1, maxLength=200 |
| `items[].reservation.startAt` | `string` | yes | 予約開始日時です。 | - |
| `items[].reservation.endAt` | `string` | yes | 予約終了日時です。この時刻を含みません。 | - |
| `items[].reservation.purpose` | `string` | yes | 予約目的です。本人と管理者だけに返します。 | minLength=1, maxLength=200 |
| `items[].reservation.status` | `string(confirmed, cancelled)` | yes | 予約の状態を表す列挙値です。 | confirmed=確定済みで重複判定の対象になる予約です。<br>cancelled=取消済みで重複判定から除く予約です。 |
| `items[].reservation.version` | `integer` | yes | 予約取消の楽観ロックに使う版です。 | minimum=1.0 |
| `nextToken` | `string \| null` | no | 次ページを取得するために前回レスポンスから受け取る継続tokenです。 | minLength=1, maxLength=512 |
...
| `error.details[].resource.resourceId` | `string \| null` | no | 予約表の対象資源の存在確認、問い合わせに使用する資源IDです。 | minLength=1, maxLength=36 |
| `error.details[].resource.day` | `string \| null` | no | 予約表を再取得するための日付です。 | - |
