...
予約できる会議室と備品を、名前とIDの固定順でページ単位に取得します。
...
| `422` | queryがOpenAPIスキーマの型や制約に一致しない、または継続tokenが不正な場合。 | `application/json` | 12 field(s) |
...
| 項目 | 型 | 必須 | 説明 | 制約 |
| --- | --- | --- | --- | --- |
| `items` | `array<ResourceItemResponse>` | yes | 一覧レスポンスに含まれるリソース配列です。 | - |
| `items[].resourceId` | `string` | yes | 予約対象の資源を一意に識別するIDです。 | minLength=1, maxLength=36 |
| `items[].name` | `string` | yes | 利用者に表示する資源名です。 | minLength=1, maxLength=100 |
| `items[].description` | `string` | yes | 資源の設備や用途を説明する文章です。 | maxLength=1000 |
| `items[].kind` | `string(room, equipment)` | yes | 予約対象の資源種別を表す列挙値です。 | room=会議室です。<br>equipment=持ち運びできる備品です。 |
| `items[].active` | `boolean` | yes | 新規予約を受け付けるかどうかです。 | - |
| `items[].version` | `integer` | yes | 資源編集の楽観ロックに使う公開版です。 | minimum=1.0 |
| `nextToken` | `string \| null` | no | 次ページを取得するために前回レスポンスから受け取る継続tokenです。 | minLength=1, maxLength=512 |
...
| `error.details[].resource.nextToken` | `string \| null` | no | 一覧復帰時に同じ位置から再取得するための継続tokenです。 | minLength=1, maxLength=512 |
