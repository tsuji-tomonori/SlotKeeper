...
APIはlazunex形式の運用ログを1行1JSONで出力します。要求ごとに`messageId=http.request_completed`（catalog `H001`）を`traceId`、`api.operation`、`api.statusCode`、`metrics.durationMs`付きで記録し、業務・Router例外はAPIごとのmessage catalog（`docs/spec/40.apis/<group>/<api>/messages_gen.md`、一覧は`messages_index_gen.md`）のIDで記録します。応答ヘッダー `X-Request-ID` とエラー本文の `error.traceId` は `traceId` と一致します。予約目的・token・生の例外本文は出力しません。

```bash
docker compose logs api --since 10m | grep '"traceId":"<X-Request-ID>"'
```
