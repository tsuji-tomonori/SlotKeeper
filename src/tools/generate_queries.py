...
def output_field(
    ...
    if isinstance(expression, exp.Alias):
        ...
        if isinstance(expression.this, exp.Count):
            return FieldSpec(name=alias, type_hint="int")
...
def parameter_name(expression: Any) -> str | None:
    if isinstance(expression, exp.Cast):
        return parameter_name(expression.this)
...
def cast_type_hint(data_type: exp.DataType) -> str:
    return base_type_from_sql(data_type.sql(dialect="postgres"))


def collect_cast_param_types(statement: Any) -> dict[str, str]:
    """CAST(@name AS type) で明示した束縛引数の型を取得する。"""
    param_types: dict[str, str] = {}
    for cast in statement.find_all(exp.Cast):
        name = parameter_name(cast.this)
        if name is not None:
            param_types[name] = cast_type_hint(cast.to)
    return param_types
...
def collect_comparison_param_columns(
    ...
    param_columns: dict[str, Column] = {}
    aliases = table_aliases(statement)
    for comparison in statement.find_all(exp.EQ, exp.NEQ, exp.LT, exp.LTE, exp.GT, exp.GTE):
...
def infer_param_fields(
    ...
    for statement in statements:
        cast_types.update(collect_cast_param_types(statement))
        if isinstance(statement, exp.Insert):
    ...
    for name in placeholder_names(sql):
        ...
        if column is None and name in cast_types:
            fields.append(
                FieldSpec(
                    name=python_identifier(name),
                    type_hint=cast_types[name],
...
def unresolved_fields(spec: QuerySpec) -> list[str]:
    return [field.name for field in [*spec.params, *spec.rows] if field.type_hint == "Any"]
...
def parse_query_spec(sql_path: Path, tables: dict[str, Table]) -> QuerySpec:
    ...
    spec = build_query_spec(sql_path, sql, statements, tables)
    unresolved = unresolved_fields(spec)
    if unresolved:
        raise ValueError(
            f"{sql_path}: DDL列またはCASTで型を決定できない項目: {', '.join(unresolved)}"
        )
    return spec


def build_query_spec(
    sql_path: Path, sql: str, statements: list[Any], tables: dict[str, Table]
) -> QuerySpec:
    return QuerySpec(
