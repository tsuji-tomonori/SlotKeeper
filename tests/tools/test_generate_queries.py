...
def test_unknown_placeholder_type_is_rejected(tmp_path: Path) -> None:
    sql_path = tmp_path / "001_select_projects.sql"
    sql_path.write_text(
    ...
    with pytest.raises(ValueError, match="unknown_value"):
        parse_query_spec(sql_path, parse_tables(DDL))


def test_cast_placeholder_type_is_inferred(tmp_path: Path) -> None:
    sql_path = tmp_path / "001_select_projects.sql"
    sql_path.write_text(
        "SELECT project_id FROM projects WHERE CAST(@after_code AS TEXT) IS NULL;",
        encoding="utf-8",
    )

    spec = parse_query_spec(sql_path, parse_tables(DDL))

    assert [(field.name, field.type_hint) for field in spec.params] == [("after_code", "str")]
