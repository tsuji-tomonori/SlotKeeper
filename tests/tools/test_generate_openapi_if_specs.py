...
def test_schema_type_and_constraints() -> None:
    ...
    assert (
        schema_constraints(
            {"type": "string", "enum": ["room"], "title": "ResourceKind"},
            schemas,
        )
        == "room=会議室です。"
    )
...
def test_operation_samples_are_loaded_from_samples_module() -> None:
    api_path = Path("reservations/create_reservation")

    assert sample_module_name(api_path) == "app.apis.reservations.create_reservation.samples"
    ...
    assert sample.request["purpose"] == "定例会議"
    assert sample.response["status"] == "confirmed"
    assert sample.status_samples[201]["response"]["status"] == "confirmed"
    assert sample.error_resource_schema is not None
    assert "resourceId" in sample.error_resource_schema["properties"]
    assert "idempotencyKey" in sample.error_resource_schema["properties"]
    assert (
        implementation_operation_samples({"createReservation": api_path})["createReservation"]
        == sample
    )
